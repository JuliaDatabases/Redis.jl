...
function parse_simple_string_reply_test()
    ...
    response = parse_redis_reply(reply)
...
function parse_error_reply_test()
    ...
    @test_throws ServerException parse_redis_reply(reply)
end

function parse_redis_reply_malformed_test()
    reply = "badreply"
    @test_throws ProtocolException parse_redis_reply(reply)
...
parse_redis_reply_malformed_test()
pack_command_test()